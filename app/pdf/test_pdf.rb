class TestPdf < Prawn::Document
  def initialize(kanji_ids)
    ...
    kanji_ids.each_with_index do |kanji_id, i|
      ...
      kana = Reading.find(kanji_id).kana
      case kanji_id
      when '1'
      ...
      when '6'
        text_box kana[0], at: [start_x_position, start_y_position + 20], width: 20, height: 40, align: :center, valign: :center
        text_box kana[1], at: [start_x_position, start_y_position - 20], width: 20, height: 40, align: :center, valign: :center
        text_box Reading.find(kanji_id).okurigana, at: [start_x_position - 90, start_y_position - 60], width: 80, height: 80, size: 50, align: :center, valign: :center
      end
